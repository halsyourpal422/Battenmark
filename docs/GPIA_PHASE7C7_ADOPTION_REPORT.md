...
## Frozen-Scenario Referential-Parity Remediation

A second independent review accepted the external project guard, instance-transform mutation, and native definition-parameter validation, then identified two remaining evaluator-only gaps: enclosure feature references still fell through to generic success, and the backend diagnostic error was injected without the public project/document/body session in which it could have occurred.

The existing symbol-hidden evaluation registry remains the single state model. It now stores only bounded public feature data: stable feature id, name, kind, owning body, and, for sketches, plane, origin, and deterministic rectangle/circle profile records. `create_sketch`, `add_rectangle`, `add_circle`, `pocket`, and `pad` mirror canonical reference failures before mutation. Boolean and alias operations resolve both returned bodies, reject equal target/tool identities, record the mapped operation, and set `feature_applied` only after success. Bounded public result data is retained with tool-call evidence so the enclosure scorer can connect a successful subtractive operation to the two bodies actually returned by `create_box`.

Backend diagnostics uses a deterministic fixture prelude. Before the promised failed call is shown to the model, the prelude seeds the same hidden registry with public project `diagnostic-project`, document `diagnostic-document`, and body `diagnostic_fixture`. The synthetic failed-result envelope exposes those three safe handles, and its recorded arguments include the project handle exactly as a project-scoped request would. No registry object is serialized. Evaluation-only `query_geometry` and `inspect_faces` reject unknown bodies, preserve `gref_missing` as `GEOMETRY_REFERENCE_LOST`, and return one bounded deterministic `top_face` match for the seeded diagnostic body. This is public-result/session parity, not OCC simulation.

Evaluation semantics remain `battenmark.phase7c.identity-integrity.v4`: no paid v4 baseline exists, tool schemas did not change, and the new Git SHA independently invalidates old checkpoints. The scorer weights, frozen scenarios, skills, provider/model configuration, and production CAD/service behavior remain unchanged. The only scorer logic adjustment requires an actually successful feature call instead of granting `feature_applied` from a failed call name.
