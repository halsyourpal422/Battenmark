...
const ASSEMBLY_TOOLS: CatalogEntry[] = [
  ...
  assemblyEntry("set_parallel", "Make two planar faces parallel. Pure orientation: the second referenced instance rotates minimally about its current position; translation is preserved. Planar faces only — axis references are not supported by this operation.", { assembly_id: { type: "string" }, a_instance: { type: "string" }, a_ref: assemblyRefFace, b_instance: { type: "string" }, b_ref: assemblyRefFace }, ["assembly_id","a_instance","a_ref","b_instance","b_ref"]),
  assemblyEntry("set_perpendicular", "Make two planar faces perpendicular (dot of normals = 0) via minimal deterministic rotation of the second referenced instance. Planar faces only.", { assembly_id: { type: "string" }, a_instance: { type: "string" }, a_ref: assemblyRefFace, b_instance: { type: "string" }, b_ref: assemblyRefFace }, ["assembly_id","a_instance","a_ref","b_instance","b_ref"]),
  entry({
    name: "check_interference",
    description: "Authoritative static interference check through FreeCAD/OpenCascade B-rep intersections. Volumetric only — face contact is not interference. AABB broad-phase pre-filters pairs.",
    properties: props({ assembly_id: { type: "string" }, min_volume_mm3: { type: "number", description: "Report only intersections above this volume (default ~1e-6 mm³)." }, instance_ids: { type: "array", items: { type: "string" }, description: "Optional scope restriction." } }),
    required: ["assembly_id"],
    kind: "read",
    permission: "cad:read",
    needsProject: true,
    mapsTo: "check_interference",
    exposeToGrok: true,
    destructive: false,
    idempotent: true,
  }),
