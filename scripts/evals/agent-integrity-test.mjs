...
import { resolveLayerBConfig } from "./run.mjs";
...
function refuseCode(env, extraArgs = []) {
  const result = spawnSync(
    "npx",
    ["tsx", "scripts/evals/run.mjs", "--mode", "agent", "--scenario", "assembly", "--condition", "both", "--repeats", "1", ...extraArgs],
    {
      cwd: root,
...
await test("real-agent-refuses-explicit-mock", () => {
  try {
    resolveLayerBConfig({
      env: { BATTENMARK_EVAL_PROVIDER: "mock", OPENAI_API_KEY: "BATTENMARK_TEST_SECRET_DO_NOT_LEAK" },
      authorizePaid: true,
    });
    throw new Error("expected throw");
  } catch (err) {
...
await test("cli-refuses-implicit-mock", () => {
  ...
  assert(/REAL_AGENT_PROVIDER_REQUIRED/.test(result.stderr + result.stdout), result.stderr + result.stdout);
});

await test("mock-rows-excluded-from-layer-b-aggregate", () => {
  const mixed = [
    { scenario_id: "assembly-planar-001", condition: "no-skill", score: 90, execution_mode: "mock-agent", provider: "mock" },
    { scenario_id: "assembly-planar-001", condition: "with-skill", score: 99, execution_mode: "mock-agent", provider: "mock" },
    { scenario_id: "assembly-planar-001", condition: "no-skill", score: 40, execution_mode: "real-agent", provider: "openai-compatible" },
    { scenario_id: "assembly-planar-001", condition: "with-skill", score: 70, execution_mode: "real-agent", provider: "openai-compatible" },
  ];
